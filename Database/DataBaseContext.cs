...
namespace Kamanri.Database
{
	...
	public sealed class DatabaseContext
	{
		...
		{
			Te.EntityAttributesString.Build();
			string SQLStatement = $"insert into {Te.TableName} ({Te.EntityAttributesString.ColumnNamesString}) values ({Te.EntityAttributesString.InsertValuesString})";
		...
		{
			Te.EntityAttributesString.Build();
			string SQLStatement = $"insert into {Te.TableName} ({Te.Columns()}) values ({Te.ID},{Te.EntityAttributesString.InsertValuesString})";
			await _sql.Execute(SQLStatement, command => Te.SetParameter(command));
		}
		...
		public async Task Inserts<TEntity>(IList<Entity<TEntity>> Tes)
		{
			...
			string insertValuesString = "",tableName = Tes[0].TableName,columnNamesString = Tes[0].EntityAttributesString.ColumnNamesString;
			for(int i = 1;i < Tes.Count - 1;i++)
			{
				
				insertValuesString += $"({Tes[i].EntityAttributesString.InsertValuesString})";
				insertValuesString += ",";
			}
			insertValuesString += $"({Tes[Tes.Count - 1].EntityAttributesString.InsertValuesString})";
		...
		public async Task InsertsWithID<TEntity>(IList<Entity<TEntity>> Tes)
		{
			...
			for(int i = 1;i < Tes.Count - 1;i++)
			{
				insertValuesString += $"({Tes[i].ID},{Tes[i].EntityAttributesString.InsertValuesString})";
				insertValuesString += ",";
			}
			insertValuesString += $"({Tes[Tes.Count - 1].ID},{Tes[Tes.Count - 1].EntityAttributesString.InsertValuesString})";
		...
		{
			Te.EntityAttributesString.Build();
			string SQLStatement = $"update {Te.TableName} set {Te.EntityAttributesString.UpdateSetString} where ID = {Te.ID}";
			await _sql.Execute(SQLStatement,command => Te.SetParameter(command));
		}
		...
		{
			Te.EntityAttributesString.Build();
			string SQLStatement = $"select * from {Te.TableName} where {Te.EntityAttributesString.CandidateKeySelectionString}";
		...
		public async Task<IList<TEntity>> SelectIDs<TEntity>(IList<Entity<TEntity>> Tes)
		{
			...
			for (int i = 0; i < Tes.Count - 1;i++)
			{
				constraints += $"({Tes[i].EntityAttributesString.CandidateKeySelectionString})";
				constraints += " or ";
			}
			constraints += $"({Tes[Tes.Count - 1].EntityAttributesString.CandidateKeySelectionString})";
